...
namespace _1._3._30
{
    public class DataGridViewUtils
    {
        ...
        public static T[,] GridToArray2<T>(DataGridView dgv)
        {
         ...
         public static List<LogicLibrary1.Student> GridToStudents(DataGridView dgv)
        {
            List<LogicLibrary1.Student> students = new List<LogicLibrary1.Student>();
            int n = dgv.RowCount;
            for (int i = 0; i < n; i++)
            {
                if (dgv[0, i].Value != null)
                {
                    ...
                    double[] medb = new double[medbs.Length];
                    for (int j = 0; j < medb.Length; j++)
                    {
                        medb[j] = Convert.ToDouble(medbs[j]);
                    }
                    byte kurs = Convert.ToByte(dgv[3, i].Value);
                    byte group = Convert.ToByte(dgv[4, i].Value);
                    LogicLibrary1.Student st = new LogicLibrary1.Student() {FIO= fio,Year= year,MedB= medb,Kurs= kurs,Group= group };
                    students.Add(st);
                }
            }
            return students;
        ...
        public static void StudentToGrid(List<LogicLibrary1.Student> students,DataGridView dgv)
        {
            ...
            int i = 0;
            foreach( LogicLibrary1.Student std in students)
            {
                //if (i != 11)
                    //dgv.Rows.Add();
        ...
        //    }
        //}
        public static void KursesToGrid(List<LogicLibrary1.Kurs> kurses, DataGridView dgv)
        {
            int i = 0;
            foreach (LogicLibrary1.Kurs kurs in kurses)
            {
                dgv.Rows.Add();
                dgv[0, i].Value = "Курс " + kurs.NumberKurs;
                i++;
                foreach (LogicLibrary1.Student std in kurs.Students)
                {
                    dgv.Rows.Add();
                    dgv[0, i].Value = std.FIO;
                    dgv[1, i].Value = std.Year;
